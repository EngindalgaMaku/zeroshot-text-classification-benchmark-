"""Generate V6 descriptions for ALL datasets."""
...
# V6 Prompt Template
PROMPT_V6_TEMPLATE = """You are generating a text classification description for {dataset_domain}.
...
def generate_description(dataset_name: str, dataset_domain: str, label_name: str, client: OpenAI, model: str) -> str:
    """Generate single V6 description."""
    prompt = PROMPT_V6_TEMPLATE.format(
    ...
    if not description.startswith("This text is about"):
        print(f"    ⚠️  Format warning: {description[:50]}...")
...
def main():
    ...
    for dataset_name, config in DATASETS.items():
        ...
        for label_id, label_name in config["labels"].items():
            ...
            dataset_descriptions[str(label_id)] = {"l2": description}
            print(f"   → {description}")
        
        all_descriptions[dataset_name] = dataset_descriptions
    
    # Save
    output_dir = Path("src/label_descriptions")
    output_file = output_dir / "generated_descriptions_v6_all.json"
    ...
    print("2. Copy to generated_descriptions.json")
    print("3. Commit and push")
    print("4. Test in Colab with all models")
